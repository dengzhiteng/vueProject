            ...
            <div v-if="item.attrs.length > 0">
              ...
              <div class="attrItem">
                <span v-for="attrItem in item.attrs" class="attr">
                  <input
                    type="checkbox"
                    :value="attrItem.column_comment"
                    v-model="checkedList"
                    @change="handlekAttr($event, item)"
                  /><span :title="attrItem.column_comment">{{
                    attrItem.column_comment.length > 8
                      ? attrItem.column_comment.substring(0, 8) + "..."
                      : attrItem.column_comment
                  }}</span>
                  <input
                    type="text"
                    v-if="attrItem.listShowOrder != null"
                    :value="attrItem.listShowOrder"
                    @change="showConfigOrder($event, attrItem)"
                    class="order"
                  />
                </span>
              </div>
    ...
    checkedList() {
      this.allAttrs.forEach((item, index) => {
        if (this.checkedList.indexOf(item.column_comment) != -1) {
          item.listShowOrder =
            this.checkedList.indexOf(item.column_comment) + 1;
        } else {
  ...
  methods: {
    ...
    saveTem() {
      ...
      if (this.curTemId) {
        ...
        this.$post("/api/config/template/edit", params)
          .then((res) => {
        ...
        this.$post("/api/config/template/save", params)
          .then((res) => {
            this.btnLoading = false;
            if (res.success) {
    ...
    getTemList() {
      this.temList = [];
      this.loading = true;
      this.$get("/api/config/template/list/get", {
        module: this.moduleName,
        mouldName: this.searchName,
      }).then((res) => {
        this.loading = false;
